...
const Destinations2 = () => {
  ...
  const handleSubmit = async () => {
    ...
    const storageRef = ref(storage, `destinations2/${randomId}`);
    await uploadBytes(storageRef, image).then((snapshot) => {
      console.log("uploaded");
    });
    const downloadUrl = await getDownloadURL(
      ref(storage, `destinations2/${randomId}`)
    );

    await addDoc(collection(db, "destinations2"), { ...details, downloadUrl });
    setLoading(false);
    window.location.reload();
  };

  const handleDelete = async (id) => {
    await deleteDoc(doc(db, "destinations2", id));
    window.location.reload();
  };
  ...
  return (
    <div className="destinations">
      <div className="destinations-top">
        <h1>Manage Destinations Type - 2</h1>
